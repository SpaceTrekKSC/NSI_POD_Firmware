 ...
 * BME280 Environmental Sensor Example (I2C)
 *
 * This POD reads a Bosch BME280 temperature / humidity / barometric
 * pressure sensor over I2C and reports the data plus an approximate
 * pressure-altitude back to the Flight Computer (FC).
 *
 * ---------------------------------------------------------------------------
 * WIRING (Arduino Uno / MEGA 2560, I2C bus):
 *   BME280 VIN  -> 3.3V or 5V   (most breakout boards have an onboard
 *                                regulator + level shifter, so 5V is OK)
 *   BME280 GND  -> GND
 *   BME280 SDA  -> A4  (Uno SDA)   [MEGA 2560: SDA = pin 20]
 *   BME280 SCL  -> A5  (Uno SCL)   [MEGA 2560: SCL = pin 21]
 *   No external pull-ups are required for a typical breakout board (they
 *   are usually populated on the module). I2C is a shared 2-wire bus.
 *
 * I2C ADDRESS:
 *   Default is 0x76. Some boards (Adafruit / a strapped SDO pin) use 0x77.
 *   setup() tries 0x76 first and then 0x77; if neither responds it prints
 *   a note and continues (non-blocking) so the FC link still works.
 *
 * REQUIRED ARDUINO LIBRARIES (install via Library Manager):
 *   - "Adafruit BME280 Library"
 *   - "Adafruit Unified Sensor"   (dependency of the BME280 library)
 *   (Wire is part of the Arduino core, no install needed.)
 *
 * DATA CHANNELS SENT TO THE FC:
 *   val_01 = temperature        (degrees C)
 *   val_02 = relative humidity  (%)
 *   val_03 = barometric pressure(hPa)
 *   val_04 = approx altitude    (m, relative to SEALEVELHPA)
 *   val_05..val_10 = unused (0.0)
 *
...
#define SEALEVELHPA   1013.25           //Standard sea-level pressure (hPa) used for the approx altitude calculation
#define BME280_ADDR_A 0x76              //Default BME280 I2C address
#define BME280_ADDR_B 0x77              //Alternate BME280 I2C address (some boards)
...
bool bmeReady = false;                //true once the sensor has been found on the I2C bus
...
void setup(){
  ...
  //Try the default I2C address first, then the alternate. Do NOT block if
  //the sensor is missing; the FC link must keep working regardless.
  if(bme.begin(BME280_ADDR_A)){
    bmeReady = true;
  }
  else if(bme.begin(BME280_ADDR_B)){
    bmeReady = true;
  }
  else{
    bmeReady = false;
...
void loop() {
  ...
  if((millis() - timer) >= TIMER_TIME){           //check if enough time has passed for the timer
    ...
    //The BME280 is a fast I2C device so we can read it directly here.
    //Only read if the sensor was found; otherwise leave the channels at 0.0.
    if(bmeReady){
      val_01.value = bme.readTemperature();          //temperature in degrees C
      val_02.value = bme.readHumidity();             //relative humidity in %
      val_03.value = bme.readPressure() / 100.0F;    //pressure in hPa (readPressure() returns Pa)
      val_04.value = bme.readAltitude(SEALEVELHPA);  //approx altitude in m relative to SEALEVELHPA
    }
