 ...
 * RANDOM TEST-DATA program for an Arduino Uno or MEGA 2560 with
 ...
 *
 * *** THIS IS A TEST SKETCH - IT USES NO REAL SENSOR ***
 * It fills all ten channels (val_01..val_10) with changing random
 * numbers so you can verify the whole data chain works BEFORE
 * connecting any real sensor:
 *     POD  ->  Flight Computer  ->  Mission Control
 * Flash this first.  Once you see the ten channels changing in
 * Mission Control you know the pod hardware, the XBee link, and the
 * FC polling all work.  Then switch to a real sensor sketch.
 *
...
//Included Libraries
#include <Arduino.h>
//No additional libraries are needed - this random test sketch uses only random()
...
//This is a random test sketch, so there are no sensor objects to set up.
...
void setup(){
  ...
  //Seed the random number generator from a floating analog pin so the
  //test values are different each time the board powers up.
...
void loop() {
  ...
  if((millis() - timer) >= TIMER_TIME){           //check if enough time has passed for the timer
    ...
    //Fill all ten channels with fresh random test values.
    //There is no real sensor here - this just proves the data chain works.
    randomData();
  }
...
//randomData() fills all ten channels with changing random numbers.
//This is TEST data only - the values have no physical meaning.  It lets
//you confirm the POD -> Flight Computer -> Mission Control chain works
//before you connect a real sensor.
void randomData(){
  val_01.value = 3.14159 * random(1000);
  val_02.value = random(100) * random(200);
  val_03.value = random(100) * random(500);
  val_04.value = random(100) * random(600);
  val_05.value = random(100) * random(300);
  val_06.value = random(100) * random(400);
  val_07.value = random(100) * random(420);
  val_08.value = random(100) * random(550);
  val_09.value = random(100) * random(990);
  val_10.value = random(100) * random(1000);
}
